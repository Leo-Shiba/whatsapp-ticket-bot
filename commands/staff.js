...
module.exports = {
  nome: 'staff',
  aliases: ['atendentes'],
  descricao: 'Gerencia atendentes: !staff (lista) | !staff add @pessoa | !staff remover @pessoa',
  apenasAdmin: true,
  async executar({ sock, msg, jid, args, db }) {
    ...
    if (acao === 'add' || acao === 'remover' || acao === 'del') {
      ...
      if (!alvo) {
        await sock.sendMessage(jid, { text: '⚙️ Marque a pessoa: *!staff add @pessoa* ou *!staff remover @pessoa*' });
        return;
      }
      if (acao === 'add') {
        db.addStaff(alvo);
    ...
    if (!staff.length) {
      await sock.sendMessage(jid, { text: '📭 Nenhum atendente cadastrado. Use *!staff add @pessoa*.' });
      return;
    }
